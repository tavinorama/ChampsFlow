...
| G7 | GEO Score computation | Contract performance — compute 3-vector TrustIndex Score (BRAND, PERFORMANCE, AI) | Art. 6(1)(b) — contract | Art. 7(V) — contract | B2B customers | Computed scores (numerical); provider_breakdown JSON (measured/baseline labels) | Supabase (DB) | Same as G1 | Same as G1 | Score records: 12 months rolling |
