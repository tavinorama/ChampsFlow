...
## TL;DR
> **Rewritten 2026-08-17 (night) to current reality** (source: `docs/company/STATE.md`).

Product **LAUNCHED, live in production** (Railway: web + api + worker; Supabase; merge-to-main = deploy). Shipped and live: Ozvor Search audits (5 engines, 3 scores, weekly monitoring on paid + comped), Ozvor Pages, Get-Cited Kit $29, self-serve Growth/Agency Stripe, **MCP server Phase 1 (#451)**. **Blocks D+C (2026-08-17)**: dashboard credits/upsell + AI Audit tab + Prime tab + /book intake (#490), site-wide $49 CTA (#491), aggressive nurture 0/1/2/2 across 9 sequences (#486), IG/TikTok/YT/PPC + video-live spheres (#484), Telegram approve/reject buttons (#482), robust blog + absence watch (#480 #483), **Signal Engine consumer both halves — sphere-reddit (#496, 14th graph) + "Where to show up" tab (#497)** + producer spec (#498), plus cleanup (#492 X-length fix, #493 billing docstring, #494 audit rate-limit, #495 nurture/credit tests). **api_spend.tenant_id (#489) merged + verified applied** → per-tenant cost live. **THE gap: the $49 answers 503 — tables `ai_audit_order` (#478) + `ai_tool` (#463) unmerged (SQL-confirmed absent); founder merge.** Hard rules hold: nothing degrades silently, every job auditable on Telegram, mock never ships (integrity guards fail closed), migrations only via founder-merged PRs. Open: E2E red-but-green on main (#146), webkit-mobile (#170), phantom `starter` tier cleanup (touches a CHECK migration), GA4 instrumentation. Pipeline detail: `docs/STATE.md`.
