...
## Department meta
- **Head**: vp-sales
- **Playbook**: [icp.md](icp.md) (canonical, v2) · [aistack-campaign-kit.md](aistack-campaign-kit.md) · [geo-campaign-kit.md](geo-campaign-kit.md) · [design-partner-pack.md](design-partner-pack.md) (canal B, 11/09) · [sop-dia-do-disparo.md](sop-dia-do-disparo.md) · [discovery-audit-playbook.md](discovery-audit-playbook.md) · [battlecards.md](battlecards.md) · [first-week-playbook.md](first-week-playbook.md) (§4 cold sequence is PRE-rules history — the kits govern) · [signal-shortlist.md](signal-shortlist.md)
- **Canal B (11/09)**: 3–5 design partners no Managed Visibility (US$ 750 no 1º mês, US$ 1.500/mês depois), vindos da rede do founder. O ativo é o **Design Partner Pack**: `npx tsx scripts/design-partner-pack.ts` gera um documento de 1–2 páginas por empresa a partir de uma auditoria REAL (~US$ 0,55, impresso antes e só gasto com `--confirm`). Funil próprio em `crm_contact` (`source='design-partner'`), exposto somado em `GET /api/v1/operator/crm` → `designPartnerFunnel`. **DESLIGADO até a migração `20260911000001_crm_design_partner` correr** (PR separado, sem label): até lá `source` não é gravado e um estágio do funil é recusado com 409, nunca substituído.
