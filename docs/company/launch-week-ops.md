...
| **Ozvor Pages** (live product, $99 ON) | Sales angle + claim boundaries: [icp.md → "landing-page creation angle"](../departments/sales/icp.md) · buy page: ozvor.com/local-pages · ops truths: generation is platform-key (worker `ANTHROPIC_API_KEY`), mock forbidden in prod (#122, job fails honestly), regen quota un-resettable (#121), refund auto-revokes credit (#271). Watch `pages_order` in the money pulse; a generation job failing = config incident, alert founder. |
