...
| **Hermes** (`ozvor-hermes`) | Orchestration, planning, reviews, operations, company coordination | Turns founder intent into plans/issues, reviews PRs (MEDIUM+), reclassifies risk when mislabeled, coordinates cadences | Writes code; touches production; holds secrets other than its own GitHub token |
