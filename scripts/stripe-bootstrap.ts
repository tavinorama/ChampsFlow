...
//
// Agency pricing updated 2026-07-16: $249/mo → $549/mo; annual $6,588 list.
// Founder price (30% off annual): $6,588 × 0.70 = $4,611.60 → $4,611/yr.
// Now 15 brands (was 25), repositioned as the premium tier — see PLAN_LIMITS.
//
// IMPORTANT — display/charge sync:
//   The Agency price shown in the UI is $549/mo and $4,611/yr (founder).
//   The ACTUAL CHARGE comes from the Stripe price IDs in env:
//     STRIPE_PRICE_ID_AGENCY        → must point to the $549/mo price created here
//     STRIPE_PRICE_ID_AGENCY_ANNUAL → must point to the $6,588/yr price created here
...
const CATALOG = {
  ...
  agency: { name: "Ozvor — Agency", desc: "Weekly monitoring + competitor tracking across up to 15 brands.", monthly: 54900, annual: 658800 },
...
async function main(): Promise<void> {
  ...
  // prices; new keys are versioned to force creation of the $549/$6,588 prices.
  const agencyMo = await findOrCreatePrice("agency_monthly_549_usd", { product: agency.id, currency: "usd", unit_amount: CATALOG.agency.monthly, recurring: { interval: "month" } });
  const agencyYr = await findOrCreatePrice("agency_annual_6588_usd", { product: agency.id, currency: "usd", unit_amount: CATALOG.agency.annual, recurring: { interval: "year" } });
